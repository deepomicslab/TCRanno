...
def output_topk_epts(EPTS,sfreq,AO_map,k,weight=1.0):
    ...
    j = 0
    output = []
    sorted_epts = sorted(list(EPTS.keys()))
    while j < k and len(EPTS)>0:
        m = max(EPTS.values())
        for keys in sorted_epts:
            if EPTS[keys]==m:
                ag_org = AO_map[keys][0]+'->'+AO_map[keys][1]
                if ag_org not in counted:
                    counted[ag_org]=[]
                counted[ag_org].append(keys)
        j += len(counted)
        for entries in counted:
            all_keys = (',').join(counted[entries])
            out = all_keys+' ['+entries+'] ('+str(round(m/weight,5))+', '+str(round(100*m/(sfreq*weight),2))+'%)'
            output.append(out)
            for each in counted[entries]:
                sorted_epts.remove(each)
                EPTS.pop(each)
        counted = {}
    return output
...
def tcr2ept(infile, outprefix, AO_map=None, k=30, hit_rate=0.2, limit=1e-4):
    ...
    pipeline.write('Features\tLow-freq fraction\tMed-freq fraction\tHigh-freq fraction\tEffective fraction\tComplete_Match (CM)\tPredicted_Match (PM)\n')
    ...
    freqs = df[df['Index'].str.endswith('-0')]['Frequency'].tolist()+df[df['Index'].str.endswith('-1')]['Frequency'].tolist()
    low=0
    med=0
    high=0
    for f in freqs:
        if f < 1e-4:
            ...
            high+=f
    pipeline.write('Frequency\t'+str(low)+'\t'+str(med)+'\t'+str(high)+'\t'+str(sfreq_complete+sfreq_partial)+'\t'+str(sfreq_complete)+'\t'+str(sfreq_partial)+'\n')
    pipeline.write('Weight\t-\t-\t-\t-\t1.0\t'+str(hit_rate)+'\n')
    pipeline.write(('\t').join(['----------']*7)+'\n') #pipeline.write('-'*100+'\n')
    pipeline.write('Rank\tCM:Top_Epitopes (sum_of_freq, pct%)\tPM:Top_Epitopes (sum_of_freq, pct%)\tCM+PM:Top_Epitopes (sum_of_freq, pct%)\tWeightedSum:Top_Epitopes (sum_of_freq, pct%)\n')
...
def tcr2ag(infile,outprefix,AO_map=None, k=20, hit_rate=0.2, limit=1e-4):
    ...
    pipeline.write('Weight\t-\t-\t-\t-\t1.0\t'+str(hit_rate)+'\n')
    pipeline.write(('\t').join(['----------']*7)+'\n') #pipeline.write('-'*100+'\n')
    pipeline.write('Rank\tCM:Top_Antigens (sum_of_freq, pct%)\tPM:Top_Antigens (sum_of_freq, pct%)\tCM+PM:Top_Antigens (sum_of_freq, pct%)\tWeightedSum:Top_Antigens (sum_of_freq, pct%)\n')
    for j in range(min(k,max(len(cm),len(pm),len(ts),len(ws)))):
        line = ['-','-','-','-']
        if len(cm) > j:
            line[0] = cm[j]
...
def tcr2org(infile, outprefix, AO_map=None, k=10, hit_rate=0.2, limit=1e-4):
    ...
    if AO_map is None:
        AO_map = load_AO()
    complete_matches, sfreq_complete = org_frequency_calculations(df[df['Index'].str.endswith('-0')], AO_map, limit, 1.0)
    ...
    pipeline.write('Weight\t-\t-\t-\t-\t1.0\t'+str(hit_rate)+'\n')
    pipeline.write(('\t').join(['----------']*7)+'\n') #pipeline.write('-'*100+'\n')
    pipeline.write('Rank\tCM:Top_Organisms (sum_of_freq, pct%)\tPM:Top_Organisms (sum_of_freq, pct%)\tCM+PM:Top_Organisms (sum_of_freq, pct%)\tWeightedSum:Top_Organisms (sum_of_freq, pct%)\n')
    for j in range(min(k,max(len(cm),len(pm),len(ts),len(ws)))):
        line = ['-','-','-','-']
        if len(cm) > j:
            line[0] = cm[j]
