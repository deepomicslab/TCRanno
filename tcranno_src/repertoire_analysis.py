...
def tcr2ept(infile, outprefix, AO_map=None, k=30, hit_rate=0.2, limit=1e-4):
    ...
    if AO_map is None:
        AO_map = load_AO()
    complete_matches, sfreq_complete = ept_frequency_calculations(df[df['Index'].str.contains('-0')], limit, 1.0)
    ...
    pipeline.write('Features\tComplete_Match (CM)\tPredicted_Match (PM)\tTotal\tWeighted_Sum (WS)\n')
    ...
    pipeline.write('Rank\tCM:Top_Epitopes (sum_of_frequency)\tPM:Top_Epitopes (sum_of_frequency)\tTOTAL:Top_Epitopes (sum_of_frequency)\tWS:Top_Epitopes (sum_of_frequency)\n')
...
def tcr2ag(infile,outprefix,AO_map=None, k=20, hit_rate=0.2, limit=1e-4):
    ...
    if AO_map is None:
        AO_map = load_AO()
    complete_matches, sfreq_complete = ag_frequency_calculations(df[df['Index'].str.contains('-0')], AO_map, limit, 1.0)
    ...
    pipeline.write('Weight\t1.0\t0.2\t-\t-\n')
    pipeline.write(('\t').join(['--------------------']*5)+'\n') #pipeline.write('-'*100+'\n')
    pipeline.write('Rank\tCM:Top_Antigens (sum_of_frequency)\tPM:Top_Antigens (sum_of_frequency)\tTOTAL:Top_Antigens (sum_of_frequency)\tWS:Top_Antigens (sum_of_frequency)\n')
    for j in range(min(k,max(len(cm),len(pm),len(ts),len(ws)))):
        line = ['-','-','-','-']
        if len(cm) > j:
            line[0] = cm[j]
...
def tcr2org(infile, outprefix, AO_map=None, k=10, hit_rate=0.2, limit=1e-4):
    ...
    pipeline.write('Weight\t1.0\t0.2\t-\t-\n')
    pipeline.write(('\t').join(['--------------------']*5)+'\n')
    pipeline.write('Rank\tCM:Top_Organisms (sum_of_frequency)\tPM:Top_Organisms (sum_of_frequency)\tTOTAL:Top_Organisms (sum_of_frequency)\tWS:Top_Organisms (sum_of_frequency)\n')
    for j in range(min(k,max(len(cm),len(pm),len(ts),len(ws)))):
        line = ['-','-','-','-']
        if len(cm) > j:
            line[0] = cm[j]
