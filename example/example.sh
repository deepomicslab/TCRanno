#!/bin/bash
# run tcr2tcr under single-cpu mode; to speed up with mutiple cpus, use the --t paramerter.
python3 ../run_tcr2tcr.py --infile example_input_repertoire.tsv --outprefix example --cdr3_aa_col 0 --frequency_col 1
...
python3 ../run_tcr2eao.py --infile example_tcr2tcr_output.tsv --is_tcr2tcr True --outprefix example --anno_type all --k 30
# run tcr2ept, tcr2ag, tcr2org all in one command from input repertoire
python3 ../run_tcr2eao.py --infile example_input_repertoire.tsv --is_tcr2tcr False --outprefix example2 --cdr3_aa_col 0 --frequency_col 1 --anno_type all --k 20
# landscape plot
